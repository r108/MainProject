...
public class RetailSystemDriver{
	...
	/*private String name, email, contactNumber, address, password, password2,accessLevel;*/
	...
	Product product;
	public static final Pattern VALID_CONTACT_NUMBER_REGEX = Pattern.compile("^[0].[0-9]{6,8}$", Pattern.CASE_INSENSITIVE);
	public static final Pattern VALID_ACCESS_LEVEL_REGEX = Pattern.compile("^[1-2]$", Pattern.CASE_INSENSITIVE);
	public static final Pattern VALID_EMAIL_ADDRESS_REGEX = Pattern.compile("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,6}$", Pattern.CASE_INSENSITIVE);

	public static boolean validateAccessLevel(String alStr) {
        Matcher matcher = VALID_ACCESS_LEVEL_REGEX .matcher(alStr);
        return matcher.find();
	}
	...
	public static boolean validateContactNumber(String phoneStr) {
        Matcher matcher = VALID_CONTACT_NUMBER_REGEX .matcher(phoneStr);
        return matcher.find();
	}
	...
	private void automaticallyCreateProducts(){
		product = new Product("Asus EeeePC 1015px",
				...
				"Laptop", 200, personDB.getRandomSupplier());
		...
		product = new Product("HP Chromebook 11",
				...
				"Laptop", 350, personDB.getRandomSupplier());
		...
		product = new Product("Apple iPad mini",
				...
				"Tablet", 400, personDB.getRandomSupplier());
		...
		product = new Product("Amazon Kindle Fire HDX",
				...
				"Tablet", 330, personDB.getRandomSupplier());
		...
		product = new Product("Samsung Galaxy Note ",
				...
				"Tablet", 450, personDB.getRandomSupplier());
		...
		product = new Product("Velocity Micro Vector Z25",
				...
				"Desktop", 700, personDB.getRandomSupplier());
		stockControl.addNewProductToStockList(product, 7);
		product = new Product("V3 Gaming Devastator",
				...
				"Desktop", 900, personDB.getRandomSupplier());
		...
		product = new Product("Acer Predator AG3-605-UR20",
				...
				"Desktop", 600, personDB.getRandomSupplier());
		...
		product = new Product("LG G Flex",
				...
				"Smartphone", 400, personDB.getRandomSupplier());
		stockControl.addNewProductToStockList(product, 5);
		product = new Product("Google Nexus 5",
				...
				"Smartphone", 300, personDB.getRandomSupplier());
		...
		product = new Product("Apple iPhone 5S",
				...
				"Smartphone", 220, personDB.getRandomSupplier());
		stockControl.addNewProductToStockList(product, 6);
	}
